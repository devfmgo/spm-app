@extends('layouts.app')
@section('title', 'SPM App | Edit Unit')
@section('content')
    <div class="container">
        <h3 class="text-center font-semibold text-gray-700 md:text-3xl sm:text-xl my-5">Edit Unit</h3>
        <div class="md:w-96 shadow-xl p-5 m-auto ">
            <form action="{{ route('update-unit', $data->id) }}" method="post" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="my-1">
                    ...
                    <input
                        ...
                        id="grid-first-name" type="text" name="name" value={{ $data->name }}>
                </div>
                <div class="my-1">
                    <label for="" class="font-semibold text-gray-700 text-sm">Input Warna</label>
                    <div class="flex items-center space-x-1">

                        <input type="text" name="color" id="warna" value={{ $data->color }}
                            class="appearance-none  w-full  bg-gray-100 text-gray-700 rounded py-3 px-4 mb-3 border border-gray-200 leading-tight focus:outline-none focus:bg-white focus:border-teal-300 sm:text-sm">
